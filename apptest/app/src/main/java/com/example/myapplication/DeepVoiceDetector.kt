...
class DeepVoiceDetector(
    ...
    fun detect(audioFilePath: String, threshold: Float = 0.4f): Map<String, Any?> {
        ...
        // ✅ Mel sequence 일부 로그 출력 (처음 5프레임만)
        Log.d("MelSeqDebug", "🎛 Full mel sequence [time=${fullSequence[0].size}, mels=${fullSequence[0][0].size}]")

//        for (i in 0 until minOf(50, fullSequence[0].size)) {
//            val frame = fullSequence[0][i]
//            val frameStr = frame.joinToString(prefix = "[", postfix = "]") { "%.4f".format(it) }
//            Log.d("MelSeqDebug", "Frame $i: $frameStr")
//        }

        val melImageFile = File(context.cacheDir, "${basename}_mel.png")
        MelSpectrogramGenerator.generateMelImage(waveform, melImageFile)

        val externalFile = File(context.getExternalFilesDir(null), "${basename}_mel_debug.png")
        melImageFile.copyTo(externalFile, overwrite = true)
        Log.d("MelImageCheck", "🔍 Copied to external: ${externalFile.absolutePath}")


        // 🔍 이미지가 잘 저장됐는지 로그로 확인
        if (melImageFile.exists()) {
            Log.d("MelImageCheck", "✅ Mel image saved: ${melImageFile.absolutePath}")
        } else {
            Log.e("MelImageCheck", "❌ Mel image NOT saved.")
        }
    ...
    private fun infer(seq: Array<Array<FloatArray>>, img: Array<Array<FloatArray>>, vec: FloatArray): Float {
        Log.d("InferDebug", "seq shape = [${seq.size}][${seq[0].size}][${seq[0][0].size}]")
        Log.d("InferDebug", "img shape = [${img.size}][${img[0].size}][${img[0][0].size}]")
        Log.d("InferDebug", "vec length = ${vec.size}")

        val output = Array(128) { FloatArray(2) }
//        val inputs = arrayOf(seq, arrayOf(img), arrayOf(vec))
        val inputs = arrayOf(seq, img, vec)
        val outputs = mapOf(0 to output)
        interpreter.runForMultipleInputsOutputs(inputs, outputs)
        return output[0][1]
...
//        return floatBuffer
    ...
    // ✅ 수정된 extractMelSequence()
    private fun extractMelSequence(waveform: FloatArray): Array<FloatArray> {
        val numFrames = ((waveform.size - FRAME_LENGTH) / FRAME_STEP) + 1
        if (numFrames <= 0) return Array(SEQUENCE_LEN) { FloatArray(N_MELS) { 0f } }
        ...
        for (i in 0 until numFrames) {
            ...
            for (m in 0 until N_MELS) {
                ...
                melSpectrogram[m][i] = ln(energy + 1e-6f)
            }
        }

        // [N_MELS][time] → [time][N_MELS]
        val timeSteps = melSpectrogram[0].size
        val transposed = Array(timeSteps) { t ->
            FloatArray(N_MELS) { m ->
                melSpectrogram[m][t]
            }
        }
    ...
    private fun decodeMelImageAsTensor(imageFile: File): Array<Array<FloatArray>> {
        ...
        val resizedBitmap = Bitmap.createScaledBitmap(bitmap, 500, 128, true)
